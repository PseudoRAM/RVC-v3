...
# Cog's static schema parser requires literal defaults here. Keep these aligned
# with src/defaults.py; imported constants become required API inputs.
...
class Predictor(BasePredictor):
    ...
    def predict(
        ...
        pitch_change: float = Input(default=4, description="Pitch shift in semitones. Default +4; set 0 to preserve source pitch."),
        index_rate: float = Input(default=0.75, ge=0, le=1),
        ...
        use_index: bool = Input(default=True, description="Use the voice's retrieval index when available. Enabled by default."),
